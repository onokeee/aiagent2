...
def _cross_db_tables(alias: str, meta: dict, extra: list | None = None) -> dict:
    ...
    for rel in (meta.get("relationships") or []):
        add(catalog.parse_endpoint(rel.get("from", ""), alias), "out")
        add(catalog.parse_endpoint(rel.get("to", ""), alias), "out")

    for f in db.list_db_files():
        other = db.alias_for(f)
        if other == alias:
            continue
        for rel in (catalog.load_meta(f).get("relationships") or []):
            eps = [catalog.parse_endpoint(rel.get("from", ""), other),
                   catalog.parse_endpoint(rel.get("to", ""), other)]
...
def _er_payload(path: Path, profile: dict, meta: dict) -> dict:
    ...
    borrowed = _cross_db_tables(alias, meta, meta.get("er_external") or [])
    others: dict = {}
    for f in db.list_db_files():
        a = db.alias_for(f)
        if a in borrowed:
            others[a] = {"profile": catalog.profile_db(f), "meta": catalog.load_meta(f)}
            entries.append({"alias": a, "profile": others[a]["profile"],
                            "meta": others[a]["meta"], "path": f, "editable": False})

    layout = catalog.load_layout(meta)
    nodes = []
    for i, (tname, t) in enumerate(profile["tables"].items()):
        fks = catalog.fk_columns(entries, alias, tname)
        pk = set(catalog.effective_pk(profile, meta, tname)[0])
        nid = catalog.node_id(alias, tname)
    ...
    for a, info in others.items():
        for tname, why in sorted(borrowed[a].items()):
            ...
            if t is None:
                continue
            pk = set(catalog.effective_pk(info["profile"], info["meta"], tname)[0])
            nid = catalog.node_id(a, tname)
    ...
    edges = []
    for e in catalog.collect_edges(entries):
        if (catalog.node_id(*e["from"][:2]) not in placed
                or catalog.node_id(*e["to"][:2]) not in placed):
            continue
